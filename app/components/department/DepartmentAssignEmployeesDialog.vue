<template>
  ...
  >
    <v-card class="department-assign-card" rounded="xl">
      <v-card-title class="d-flex align-center justify-space-between pa-6 pb-2">
        <span class="text-h6 font-weight-bold">
          {{ t('department.assignEmployeesTitle') }}
        </span>
        <v-btn
          icon
          variant="text"
      ...
      <v-card-text class="pa-6 pt-2">
        <div v-if="departmentName" class="text-medium-emphasis mb-4">
          {{ departmentName }}
        </div>
...
const departmentName = computed(() =>
  props.department
    ? departmentDisplayName(props.department, locale.value)
    : "",
);
...
function hydrate() {
  resetErrors();
  selectedIds.value = Array.isArray(props.department?.employee_ids)
    ? [...props.department.employee_ids]
    : [];
}
